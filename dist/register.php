...
{
    if(isset($_POST['fname'],$_POST['lname'],$_POST['email'],$_POST['password']) && !empty($_POST['fname']) && !empty($_POST['lname']) && !empty($_POST['email']) && !empty($_POST['password']))
    {
...
<body class="overflow-visible bg-blue-700">
        ...
        <div class="flex flex-col overflow-hidden bg-white rounded-md shadow-lg max md:flex-row md:flex-1 lg:max-w-screen-md">
            ...
            <div class="p-5 bg-white md:flex-1">
                ...
                if(isset($success))
                {
                    ...
                    echo '<div class="text-green-200 text-center">'.$success.'</div>';
                    header('location:login.php');
                }
			?>
                <form method="POST" action="<?php echo $_SERVER['PHP_SELF'];?>" class="flex flex-col space-y-5">
                <div class="flex flex-col space-y-1">
                        <label for="fname" class="text-sm font-semibold text-gray-500">First Name</label>
                        <input type="text" name="fname" id="fname" autofocus class="px-4 py-2 transition duration-300 border border-gray-300 rounded focus:border-transparent focus:outline-none focus:ring-4 focus:ring-blue-200" />
                    </div>
                    <div class="flex flex-col space-y-1">
                        <label for="lname" class="text-sm font-semibold text-gray-500">Last Name</label>
                        <input type="text" name="lname" id="lname" autofocus class="px-4 py-2 transition duration-300 border border-gray-300 rounded focus:border-transparent focus:outline-none focus:ring-4 focus:ring-blue-200" />
                    </div>
                    <div class="flex flex-col space-y-1">
                    ...
                    <div>
                        <button type="submit" name="submit" class="w-full px-4 py-2 text-lg font-semibold text-white transition-colors duration-300 bg-blue-500 rounded-md shadow hover:bg-blue-600 focus:outline-none focus:ring-blue-200 focus:ring-4">
                  Register
                </button>
                    </div>
            ...
            <div class="p-4 py-6 text-white bg-green-500 md:w-80 md:flex-shrink-0 md:flex md:flex-col md:items-center md:justify-evenly">
                <div class="my-3 text-4xl font-bold tracking-wider text-center">
                    <a href="#">Heey There, You are new here!</a>
                </div>
                <p class="mt-6 font-normal text-center text-gray-300 md:mt-0">
                    We are so happy to see you here among us, by creating your account you will be able to use and serve all aour services
                </p>
                ...
                <p class="mt-6 text-sm text-center text-gray-300">
                    this template created by Hakkou Mouataz
                </p>
            </div>
        </div>
    </div>
